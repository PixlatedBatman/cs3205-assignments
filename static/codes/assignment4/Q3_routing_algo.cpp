...
static int runUntilConvergence(vector<RoutingNode *> nd)
{
    bool changed = true;
    int rounds = 0;
...
static bool failFirstNetworkLink(vector<RoutingNode *> nd)
{
    for (RoutingNode *node : nd)
    {
        if (node->failFirstAvailableLink())
            return true;
    }
...
{
    int initialRounds = runUntilConvergence(nd);

    /*Print routing table entries after routing algo converges */
    printf("Printing the routing tables after the convergence \n");
    printRT(nd);

    cout << "Initial convergence rounds: " << initialRounds << endl;

    if (!failFirstNetworkLink(nd))
    {
        cout << "No link available to fail" << endl;
        return;
    }

    int reconvergenceRounds = runUntilConvergence(nd);

    printf("Printing the routing tables after link failure reconvergence \n");
    printRT(nd);
    cout << "Rounds to converge after link failure: " << reconvergenceRounds << endl;
}
