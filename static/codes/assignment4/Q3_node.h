...
public:
    ...
    bool failLocalLinkTo(const string &neighborIp)
    {
        bool linkFound = false;

        for (size_t i = 0; i < interfaces.size(); ++i)
        {
            if (get<0>(interfaces[i]).getConnectedIp() == neighborIp)
            {
                get<2>(interfaces[i]) = INFINITY_COST;
                linkFound = true;
            }
        }
        ...
        for (size_t i = 0; i < mytbl.tbl.size(); ++i)
        {
            if (mytbl.tbl[i].nexthop == neighborIp && !isMyInterface(mytbl.tbl[i].dstip))
            {
                mytbl.tbl[i].cost = INFINITY_COST;
            }
        }
    ...
    bool failFirstAvailableLink()
    {
        for (size_t i = 0; i < interfaces.size(); ++i)
        {
            if (get<2>(interfaces[i]) >= INFINITY_COST)
                continue;

            string localIp = get<0>(interfaces[i]).getip();
            string neighborIp = get<0>(interfaces[i]).getConnectedIp();
            Node *neighbor = get<1>(interfaces[i]);

            bool failedHere = failLocalLinkTo(neighborIp);
            bool failedThere = neighbor->failLocalLinkTo(localIp);

            if (failedHere || failedThere)
            {
                cout << "Simulating link failure: " << localIp << " <-> " << neighborIp << endl;
                return true;
            }
        }
