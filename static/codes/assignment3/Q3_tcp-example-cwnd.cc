...
main(int argc, char* argv[])
{
    ...
    BulkSendHelper bulkSender2("ns3::TcpSocketFactory", sinkAddress);
    bulkSender2.SetAttribute("MaxBytes", UintegerValue(0));

    ApplicationContainer sourceApps2a = bulkSender2.Install(n0r1.Get(0));
    sourceApps2a.Start(Seconds(1.0));
    sourceApps2a.Stop(Seconds(simulation_time));

    BulkSendHelper bulkSender3("ns3::TcpSocketFactory", sinkAddress);
    bulkSender3.SetAttribute("MaxBytes", UintegerValue(0));

    ApplicationContainer sourceApps2b = bulkSender3.Install(n0r1.Get(0));
    sourceApps2b.Start(Seconds(1.0));
    sourceApps2b.Stop(Seconds(simulation_time));
